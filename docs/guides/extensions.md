...
두 경로 모두 패널 헤더의 **'최대화'** 버튼으로 본문을 뷰포트 전체로 키울 수 있다
(`ArtifactPanel.svelte` 의 최대화 모드, 복귀는 상단 hover 복귀 버튼).

호스트 테마(다크/라이트)는 iframe src 의 `?theme=` + `BroadcastChannel("app:theme")` 로
확장에 동기화된다 — 확장은 mount 시 쿼리로 초기 테마를 적용하고 방송을 구독해 `data-theme`
를 갱신한다(iframe 은 부모 CSS 변수를 상속하지 않으므로 자체 적용). evaluator 참고:
`extensions/evaluator/frontend/src/lib/theme.svelte.js`.
