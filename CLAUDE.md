...
| H (큐레이션 핸드오프) | `순위 검토`, `후보 큐레이션`, `큐레이션 도구` | `exec_code`→`save_artifact`(parquet)→`open_curation` 진입 카드(markdown 칩)·새 탭 `/ext/evaluator/?bundle=` 링크 (`rank_review` SKILL 계약) |
...
예시: [evaluator](extensions/evaluator/) (parquet 큐레이션 UI). 상세 → `.claude/rules/extensions_architecture.md`

**진입 규약** — 에이전트가 `open_curation(tool, sources, mapping)` 도구를 호출하면 번들 스펙
(`<tool>.bundle.json`)을 쓰고 **markdown 칩**(진입 카드)으로 `/ext/<tool>/?bundle=` 새 탭
링크를 표시한다(전용 프론트 컴포넌트 없이 `display_markdown` 칩 경로 재사용; 새 탭은
`lib/markdown.js` DOMPurify 훅이 모든 링크에 부여). evaluator 는 `?path=`(단일) 또는
...
evaluator UI 는 좌측 선택 리스트(**너비 조절·기본 전체 선택·Ctrl+클릭 다중 표시·검색/legend
필터/일괄 선택**)와 본문 **보기 모드(항목별 ↔ 전체 조망) + 차트 종류 셀렉터 + 매핑 설정 모달 +
차트 그리드**(메인 앱 `display_chart` UX 를 클라이언트에서 자체 구현 — Tableau 풍 BI 컨셉)로
구성된다. **차트 종류**는 `display_chart` 와 동일 7종(scatter/line/bar/box/histogram/ecdf/heatmap)을
셀렉터로 전환하며, 종류를 바꾸면 **필요한 차트별 매핑 역할(x/y/집계)도 달라진다**(공통 역할
select/sort/legend/desc 는 항상 동일). **전체 조망**은 가시(필터된) 후보를 한 차트에 모아(항목=시리즈)
어느 항목을 남길지 비교한다(읽기전용). **매핑 설정 모달**은 역할별 설명 + 컬럼 드롭다운을 제공하고
...
판단을 에이전트에게 환류한다(수신부 `frontend/src/lib/evaluatorBridge.svelte.js`). 진입 카드의
`open_curation` 은 선택적 `mark` 로 기본 차트 종류도 넘길 수 있다. 상세 →
`.claude/rules/extensions_architecture.md`.
...
| `extensions_architecture.md` | 확장 시스템 — 로더·App.spec 번들·`open_curation` 진입 규약·evaluator 예시·격리 원칙 |
