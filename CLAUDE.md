...
| `APP_REPO_BASE_URL` | — | GitHub 레포 루트 URL (`_validate_meta` startswith 기준) |
| `APP_LATEST_JSON_URL` | — | latest.json 포인터 URL (GitHub: `releases/latest/download/latest.json`) |
| `APP_REPO_READ_TOKEN` | — | 읽기 전용 Classic PAT — EXE에 번들, latest.json·EXE 다운로드 인증용 |
| `APP_BUILD_CHANNEL` | (`qa`/`prod`) | App.spec이 주입 — 소스 `.env`에 두지 않음. `qa`: Mock 노출·업데이트 차단; `prod`: Mock 제외·업데이트 활성 |
