...
evaluator UI 는 좌측 선택 리스트(**너비 조절·기본 전체 선택·Ctrl+클릭 다중 표시**)와 본문
**차트 그리드**(메인 앱 `display_chart` UX 를 클라이언트에서 자체 구현 — 제목 패널·페이지네이션·
확대 라이트박스·Filter/Reset/Legend, 검토용 휘발 상태). `desc` 매핑은 선택적(컬럼 부재 시 생략).
**내보내기** 성공 시 `BroadcastChannel("evaluator:exports")` 로 메인 앱 탭에 알려 그 parquet 을
데이터 칩으로 인폼한다(수신부 `frontend/src/lib/evaluatorBridge.svelte.js`). 상세 →
