...
uv run python -m pytest backend/tests/test_subagent_isolation.py -v  # 서브에이전트 격리
...
| `APP_NEXUS_BASE_URL` | (내부) | 업데이트 저장소 URL |
| `APP_NEXUS_USER` / `_PASSWORD` | — | release.ps1 자동 업로드 자격증명 |
