...
- **리스트 검색·필터·일괄 선택**: 검색(key·desc 부분일치)·legend 필터(`legendByKey` 합집합 드롭다운)로
  ...
  **표시만** 좁히고 순서·내보내기 의미는 불변 — `order`/`selected` 는 전체 기준 유지, 순서 이동은
  `orderIndex` 로 전체 order 위치를 찾아 인접 교환, cursor 는 `filteredOrder` 기준(필터 변동 시 클램프).
- **보기 모드(항목별 ↔ 전체 조망)**: 본문 상단 토글. 조망은 `flattenForOverview(pointsByKey,
  filteredOrder)`(항목=시리즈, legend=key)를 단일 standalone `ChartCell`(읽기전용 — brush 미연결)로
  렌더해 가시 후보 전체를 한 차트에서 비교하게 한다. 조망은 per-key chartState 스냅샷을 건드리지 않는다.
...
| 파일 | 역할 |
|---|---|
| `lib/chartOption.js` | **mark 별 빌더 디스패처** `buildChartOption(mark, points, snapshot, opts)` → 7종(scatter/line/bar/box/histogram/ecdf/heatmap) ECharts option. 제외·레전드(순서·색상·Hide) 반영 + brush 환원용 `row_ids` + `brushable` 플래그 + 매핑 누락 시 친절한 `error`. line/ecdf 는 투명 scatter overlay 로 brush 우회(메인 앱 `_with_brush_overlay` 패턴), bar/box/histogram/heatmap 은 비-brush(legend Filter 만). box/histogram/ecdf 통계는 JS 로 계산(메인 앱 알고리즘과 동치 — 분위수 선형보간·10빈·누적비율). `MARKS`/`MARK_BY_ID`/`AGGREGATES` 메타데이터를 매핑 UI 와 공유 |
...
| `lib/ChartCell.svelte` | 단일 ECharts(embedded 그리드 셀 / standalone 라이트박스). `mark`·`roles`·`aggregate` props 로 빌더 분기. 매핑 누락 시 안내 문구. standalone 은 brush 활성, `onchart(chart, row_ids)` 로 라이트박스에 통지 |
