...
만드는 큐레이션 UI. 좌측 선택 리스트(체크·순서 변경·**너비 조절**) + 본문 **차트 그리드**(다중 선택
scatter·페이지네이션·확대 라이트박스·Filter/Legend) + 하단 저장/내보내기.
...
| `POST /export {path, selected, mapping, excluded}` | 선택 항목만 필터 + 차트 Filter 제외 행 실제 삭제(`excluded`: 선택키별 0-based point 인덱스) + sort 정수 재계산 → 새 parquet (내보내기) |
...
| `desc` | 리스트 보조 설명 (**선택적** — 컬럼 부재 시 desc=None 폴백, `_resolve_desc_expr`) | `item_desc` |

> `desc` 만 선택적이다 — `required_columns()` 에서 제외돼 데이터에 설명 컬럼이 없어도 큐레이션
> 진입이 막히지 않는다. 나머지 5개 역할 컬럼은 부재 시 `/dataset` 이 422.
...
- **메인 앱 실시간 인폼(export-back)**: 내보내기 성공 시 프론트가 `BroadcastChannel("evaluator:exports")`
  로 `{type:"export", session, path, filename, rows, columns}` 를 post 한다(`session` 은 소스 경로의
  `<session>`). 같은 출처인 메인 앱 탭의 `frontend/src/lib/evaluatorBridge.svelte.js` 가 구독해, 그
  세션의 **마지막 어시스턴트 메시지에 parquet 데이터 칩**을 붙이고(활성 세션이면 패널 오픈) localStorage
  영속시킨다. 칩이 메시지에 임베드되므로 새로고침 후에도 남는다. 별도 탭이므로 채팅 SSE 가 아닌
  BroadcastChannel 을 쓴다(evaluator 는 `open_curation` 규약상 새 탭에서 열림).
...
| 파일 | 역할 |
|---|---|
| `lib/chartOption.js` | `buildScatterOption(points, snapshot)` → 제외·레전드(순서·색상·Hide) 반영 ECharts option + brush 환원용 `row_ids`(시리즈별 원본 point 인덱스). 차트 자체 제목은 두지 않음(감싼 패널 헤더가 제목 표시 — 캔버스 안 중복 제거) |
...
| `lib/ChartCell.svelte` | 단일 ECharts(embedded 그리드 셀 / standalone 라이트박스). standalone 은 brush 활성, `onchart(chart, row_ids)` 로 라이트박스에 통지 |
| `lib/ChartGrid.svelte` | 제목 패널 + 페이지당 6개 페이지네이션. 셀 클릭 → 라이트박스 |
| `lib/ChartLightbox.svelte` | 리사이즈 확대 모달 + Filter/Filter All/Reset/Undo/Redo + 레전드 편집 패널. **인라인 SVG 아이콘**(evaluator 는 오프라인-퍼스트라 메인 앱의 Material Symbols CDN 미사용) |

- **표시 선택 = Ctrl/⌘+클릭**: 좌측 항목을 클릭하면 단일 표시, Ctrl/⌘+클릭으로 표시 집합 토글
  (`displaySelection`, 체크박스의 큐레이션 선택과 별개). `chartState` 식별자는 `${activePath}::${key}` 로
